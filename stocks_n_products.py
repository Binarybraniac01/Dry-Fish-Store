...
class Stockcls(Tk):
    ...
    def stk_bttons(self, btn_name, x, y):
        self.btn = Button(self.stk_prodFrame, text=btn_name, font=("Poor Richard",20, "bold"),bd=2, width=6,
                          fg="white", bg="#3F2305")
    ...
    def show_prod_data(self, event):
        ...
        if (self.len_curr_position < 7):
            ...
            if str(self.curr_position["values"][6]) == "None":
    ...
    def add_product_data(self):
        if (self.prod_name.get() == "" or self.prod_category.get() == "" or
            ...
            if result:
                ...
                try:
                    self.sql_query = (
                        "SELECT `pid`, `name`, `description`, `category`, `quantity`, `price`, `sp_price` "
                        "FROM `product_table`")
                    self.cursor.execute(self.sql_query)
                    self.results = self.cursor.fetchall()
                    # print(self.results)
                    for self.idx, self.values in enumerate(self.results):
                        # print(self.values)
                        self.P_inven_table.insert(parent="", index="end", iid=self.idx, text="Parent",
                                                  values=self.values)
                except Exception as server_error:
                    tmsg.showerror(title="Connection Error", message="Unable to connect the MySql server")
